...
public class CacheControlInstrumentation extends SimpleInstrumentation {
    ...
    static private final String DIRECTIVE_NAME = "cacheControl";
    static private final String EXTENSION_KEY = "cache-control";
    ...
    public InstrumentationContext<ExecutionResult> beginField(InstrumentationFieldParameters parameters) {
        ...
        // There's no way to set a cacheControl directive on the _entities field or the _Entity union in SDL.
        // Instead, we can determine the possible return types from the representations arguments and select the
        ...
        if (isEntityField(returnType)) {
            ...
            if (representations instanceof List) {
                typesFromEntitiesArgument(representations, parameters.getExecutionContext().getGraphQLSchema())
                        .stream()
        ...
        Optional<Hint> fieldHint = hintCache.getHint(
                parameters.getField(),
        ...
        if (fieldHint.isPresent()) {
            ...
            // If inheritMaxAge is true, take note of that to avoid setting the default max age in the next step.
    ...
    public CompletableFuture<ExecutionResult> instrumentExecutionResult(ExecutionResult executionResult, InstrumentationExecutionParameters parameters) {
        ...
        ExecutionResultImpl resultWithCacheControl = ExecutionResultImpl.newExecutionResult().from(executionResult)
    ...
    private static class CacheControlPolicy implements InstrumentationState {
        ...
        void restrict(CacheControlPolicy policy) {
            if (policy.maxAge.isPresent() && (!maxAge.isPresent() || policy.maxAge.get() < maxAge.get())) {
                this.maxAge = policy.maxAge;
            }

            if (policy.scope.isPresent() && (!scope.isPresent() || !scope.get().equals(CacheControlScope.PRIVATE))) {
        ...
        void restrict(Hint hint) {
            ...
            if (hint.scope.isPresent() && (!scope.isPresent() || !scope.get().equals(CacheControlScope.PRIVATE))) {
        ...
        public Optional<String> getPolicy() {
            ...
            return Optional.of(String.format("max-age=%d, %s", maxAgeValue, scope.orElse(CacheControlScope.PUBLIC).toString().toLowerCase()));
        }
    ...
    private static class HintCache {
        ...
        public Optional<Hint> getHint(GraphQLFieldDefinition fieldDefinition, GraphQLCompositeType parentType) {
    ...
    private static class Hint {
        ...
        public static Optional<Hint> fromDirective(GraphQLDirective directive) {
            ...
            Optional<Integer> maxAge = Optional.ofNullable(directive.getArgument("maxAge"))
                    .map(a -> GraphQLArgument.getArgumentValue(a))
                    .filter(v -> v instanceof Integer)
                    .map(Integer.class::cast);

            Optional<CacheControlScope> scope = Optional.ofNullable(directive.getArgument("scope"))
                    .map(a -> GraphQLArgument.getArgumentValue(a))
                    .filter(v -> v instanceof String)
                    .map(s -> CacheControlScope.valueOf((String) s));

            Optional<Boolean> inheritMaxAge = Optional.ofNullable(directive.getArgument("inheritMaxAge"))
                    .map(a -> GraphQLArgument.getArgumentValue(a))
        ...
        public Hint(Optional<Integer> maxAge, Optional<CacheControlScope> scope, Optional<Boolean> inheritMaxAge) {
            this.maxAge = maxAge;
            this.scope = scope;
        ...
        public String toString() {
            return String.format("@cacheControl(maxAge: %s, scope: %s, inheritMaxAge: %s)",
    ...
    static List<GraphQLType> typesFromEntitiesArgument(Object representations, GraphQLSchema schema) {
        if (representations instanceof List) {
            return ((List<?>) representations).stream()
